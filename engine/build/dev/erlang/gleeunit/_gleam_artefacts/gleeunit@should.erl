...
-spec be_ok({ok, BSX} | {error, any()}) -> BSX.
be_ok(A) ->
    case A of
        {ok, Value} ->
            Value;
...
-spec be_error({ok, any()} | {error, BTC}) -> BTC.
be_error(A) ->
    case A of
        {error, Error} ->
            Error;
...
-spec be_some(gleam@option:option(BTF)) -> BTF.
be_some(A) ->
    case A of
        {some, Value} ->
            Value;
