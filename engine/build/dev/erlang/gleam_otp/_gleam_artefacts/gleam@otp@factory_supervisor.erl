...
-opaque supervisor(AZM, AZN) :: {supervisor, gleam@erlang@process:pid_()} |
    {named_supervisor, gleam@erlang@process:name(message(AZM, AZN))}.

-type message(AZO, AZP) :: any() | {gleam_phantom, AZO, AZP}.

-opaque builder(AZQ, AZR) :: {builder,
        gleam@otp@supervision:child_type(),
        fun((AZQ) -> {ok, gleam@otp@actor:started(AZR)} |
            {error, gleam@otp@actor:start_error()}),
        gleam@otp@supervision:restart(),
        integer(),
        integer(),
        gleam@option:option(gleam@erlang@process:name(message(AZQ, AZR)))}.

-type erlang_start_flags() :: any().

-type erlang_supervisor_name(AZS, AZT) :: {local,
        gleam@erlang@process:name(message(AZS, AZT))}.

-type strategy() :: simple_one_for_one.

-type erlang_start_flag(AZU) :: {strategy, strategy()} |
    {intensity, integer()} |
    {period, integer()} |
    {gleam_phantom, AZU}.
...
-type erlang_child_spec_property(AZV, AZW) :: {id, integer()} |
    {start,
        {gleam@erlang@atom:atom_(),
            gleam@erlang@atom:atom_(),
            list(fun((AZV) -> {ok, gleam@otp@actor:started(AZW)} |
                {error, gleam@otp@actor:start_error()}))}} |
...
-spec get_by_name(gleam@erlang@process:name(message(AZX, AZY))) -> supervisor(AZX, AZY).
get_by_name(Name) ->
...
-spec named(builder(BAO, BAP), gleam@erlang@process:name(message(BAO, BAP))) -> builder(BAO, BAP).
named(Builder, Name) ->
    {builder,
...
-spec restart_tolerance(builder(BAX, BAY), integer(), integer()) -> builder(BAX, BAY).
...
-spec timeout(builder(BBD, BBE), integer()) -> builder(BBD, BBE).
timeout(Builder, Ms) ->
...
-spec restart_strategy(builder(BBJ, BBK), gleam@otp@supervision:restart()) -> builder(BBJ, BBK).
...
-spec start(builder(BBP, BBQ)) -> {ok,
        gleam@otp@actor:started(supervisor(BBP, BBQ))} |
    {error, gleam@otp@actor:start_error()}.
start(Builder) ->
...
-spec supervised(builder(BCO, BCP)) -> gleam@otp@supervision:child_specification(supervisor(BCO, BCP)).
supervised(Builder) ->
...
-spec start_child(supervisor(BCV, BCW), BCV) -> {ok,
        gleam@otp@actor:started(BCW)} |
    {error, gleam@otp@actor:start_error()}.
...
-spec start_child_callback(
    fun((BDT) -> {ok, gleam@otp@actor:started(BDU)} |
        {error, gleam@otp@actor:start_error()}),
    BDT
) -> gleam@otp@internal@result2:result2(gleam@erlang@process:pid_(), BDU, gleam@otp@actor:start_error()).
...
-spec worker_child(
    fun((BAE) -> {ok, gleam@otp@actor:started(BAF)} |
        {error, gleam@otp@actor:start_error()})
) -> builder(BAE, BAF).
...
-spec supervisor_child(
    fun((BAJ) -> {ok, gleam@otp@actor:started(BAK)} |
        {error, gleam@otp@actor:start_error()})
) -> builder(BAJ, BAK).
