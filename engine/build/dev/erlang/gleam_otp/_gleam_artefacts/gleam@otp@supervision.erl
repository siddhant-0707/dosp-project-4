...
-type child_specification(AWY) :: {child_specification,
        fun(() -> {ok, gleam@otp@actor:started(AWY)} |
...
-spec supervisor(
    fun(() -> {ok, gleam@otp@actor:started(AXE)} |
        {error, gleam@otp@actor:start_error()})
) -> child_specification(AXE).
supervisor(Start) ->
...
-spec significant(child_specification(AXJ), boolean()) -> child_specification(AXJ).
...
-spec timeout(child_specification(AXM), integer()) -> child_specification(AXM).
timeout(Child, Ms) ->
...
-spec restart(child_specification(AXP), restart()) -> child_specification(AXP).
...
-spec map_data(child_specification(AXS), fun((AXS) -> AXU)) -> child_specification(AXU).
