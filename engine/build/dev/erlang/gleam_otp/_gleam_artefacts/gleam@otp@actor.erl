...
-type message(ADQ) :: {message, ADQ} |
    {system, gleam@otp@system:system_message()} |
    {unexpected, gleam@dynamic:dynamic_()}.

-opaque next(ADR, ADS) :: {continue,
        ADR,
        gleam@option:option(gleam@erlang@process:selector(ADS))} |
    {stop, gleam@erlang@process:exit_reason()}.

-type self(ADT, ADU) :: {self,
        gleam@otp@system:mode(),
        gleam@erlang@process:pid_(),
        ADT,
        gleam@erlang@process:selector(message(ADU)),
        gleam@otp@system:debug_state(),
        fun((ADT, ADU) -> next(ADT, ADU))}.

-type started(ADV) :: {started, gleam@erlang@process:pid_(), ADV}.

-opaque initialised(ADW, ADX, ADY) :: {initialised,
        ADW,
        gleam@option:option(gleam@erlang@process:selector(ADX)),
        ADY}.

-opaque builder(ADZ, AEA, AEB) :: {builder,
        fun((gleam@erlang@process:subject(AEA)) -> {ok,
                initialised(ADZ, AEA, AEB)} |
            {error, binary()}),
        integer(),
        fun((ADZ, AEA) -> next(ADZ, AEA)),
        gleam@option:option(gleam@erlang@process:name(AEA))}.

-type start_error() :: init_timeout |
    {init_failed, binary()} |
    {init_exited, gleam@erlang@process:exit_reason()}.

-type start_init_message(AEC) :: {ack, {ok, AEC} | {error, binary()}} |
...
-spec continue(AEH) -> next(AEH, any()).
continue(State) ->
    {continue, State, none}.
...
-spec with_selector(next(AET, AEU), gleam@erlang@process:selector(AEU)) -> next(AET, AEU).
...
-spec initialised(AFA) -> initialised(AFA, any(), nil).
initialised(State) ->
...
-spec selecting(
    initialised(AFF, any(), AFH),
    gleam@erlang@process:selector(AFL)
) -> initialised(AFF, AFL, AFH).
...
-spec returning(initialised(AFQ, AFR, any()), AFW) -> initialised(AFQ, AFR, AFW).
...
-spec new(AGA) -> builder(AGA, AGB, gleam@erlang@process:subject(AGB)).
new(State) ->
...
-spec new_with_initialiser(
    integer(),
    fun((gleam@erlang@process:subject(AGG)) -> {ok, initialised(AGI, AGG, AGJ)} |
        {error, binary()})
) -> builder(AGI, AGG, AGJ).
...
-spec on_message(builder(AGS, AGT, AGU), fun((AGS, AGT) -> next(AGS, AGT))) -> builder(AGS, AGT, AGU).
...
-spec named(builder(AHD, AHE, AHF), gleam@erlang@process:name(AHE)) -> builder(AHD, AHE, AHF).
named(Builder, Name) ->
    {builder,
        erlang:element(2, Builder),
        erlang:element(3, Builder),
...
-spec select_system_messages(gleam@erlang@process:selector(message(AHS))) -> gleam@erlang@process:selector(message(AHS)).
...
-spec receive_message(self(any(), AHO)) -> message(AHO).
...
-spec initialise_actor(
    builder(any(), any(), AIL),
    gleam@erlang@process:pid_(),
    gleam@erlang@process:subject({ok, AIL} | {error, binary()})
...
-spec start(builder(any(), any(), AIY)) -> {ok, started(AIY)} |
    {error, start_error()}.
start(Builder) ->
...
-spec send(gleam@erlang@process:subject(AJF), AJF) -> nil.
send(Subject, Msg) ->
...
-spec call(
    gleam@erlang@process:subject(AJH),
    integer(),
    fun((gleam@erlang@process:subject(AJJ)) -> AJH)
) -> AJJ.
