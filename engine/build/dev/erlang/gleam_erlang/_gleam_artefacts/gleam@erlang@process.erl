...
-opaque subject(RT) :: {subject, pid_(), gleam@dynamic:dynamic_()} |
    {named_subject, name(RT)}.

-type name(RU) :: any() | {gleam_phantom, RU}.

-type do_not_leak() :: any().

-type selector(RV) :: any() | {gleam_phantom, RV}.
...
-spec 'receive'(subject(ST), integer()) -> {ok, ST} | {error, nil}.
...
-spec receive_forever(subject(TB)) -> TB.
...
-spec selector_receive(selector(TF), integer()) -> {ok, TF} | {error, nil}.
...
-spec selector_receive_forever(selector(TJ)) -> TJ.
...
-spec map_selector(selector(TL), fun((TL) -> TN)) -> selector(TN).
map_selector(A, B) ->
...
-spec merge_selector(selector(TP), selector(TP)) -> selector(TP).
merge_selector(A, B) ->
...
-spec select_map(selector(UA), subject(UC), fun((UC) -> UA)) -> selector(UA).
...
-spec select(selector(TW), subject(TW)) -> selector(TW).
...
-spec select_other(selector(UO), fun((gleam@dynamic:dynamic_()) -> UO)) -> selector(UO).
...
-spec deselect(selector(UF), subject(any())) -> selector(UF).
...
-spec select_specific_monitor(selector(VA), monitor(), fun((down()) -> VA)) -> selector(VA).
...
-spec select_monitors(selector(VD), fun((down()) -> VD)) -> selector(VD).
...
-spec select_trapped_exits(selector(TT), fun((exit_message()) -> TT)) -> selector(TT).
...
-spec deselect_specific_monitor(selector(VG), monitor()) -> selector(VG).
...
-spec send_after(subject(WC), integer(), WC) -> timer().
...
-spec perform_call(
    subject(VJ),
    fun((subject(VL)) -> VJ),
    fun((selector(VL)) -> {ok, VL} | {error, nil})
) -> VL.
...
-spec call(subject(VQ), integer(), fun((subject(VS)) -> VQ)) -> VS.
...
-spec call_forever(subject(VU), fun((subject(VW)) -> VU)) -> VW.
